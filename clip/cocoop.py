...
class CoCoOpCLIP(nn.Module):
    def __init__(self, device, classnames, criterion='cosine', arch="ViT-L/14",
                        n_ctx=16, ctx_init="a_photo_of_a", ctx_position='end', constant=[18]):
        super().__init__()
        ...
        self.constant_list = constant  ### O-TPT
    ...
    def forward_ctx(self, image_features, ctx, cons=[18]):
        N = 1
        ...
        if self.l2_norm_cal:
            ...
            l2_norm = torch.linalg.norm(cons*feature_distance, dim=-1)
            ...
            #for training
            self.l2_norm_me1an_training = l2_norm_mean
        #-----------------------------------------------------
    ...
    def forward(self, input, cons=[18]):
        if isinstance(input, Tuple):
            image_features, ctx = input
            return self.forward_ctx(image_features, ctx, cons=cons)
        else:
            return self.inference(input, cons)
